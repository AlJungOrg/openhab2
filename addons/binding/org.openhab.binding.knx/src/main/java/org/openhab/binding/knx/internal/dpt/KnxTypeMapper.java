 ...
 * between openHAB and KNX. When a command or status update is sent to an item
 * on the openHAB event bus, it must be clear, in which format it must be sent
 * to KNX and vice versa.
 *
 * @author Kai Kreuzer
 * @since 0.3.0
 */
public interface KnxTypeMapper {
    ...
    /**
     * maps an openHAB command/state to a string value which correspond to its
     * datapoint in KNX
     *
     * @param type
     *            a command or state
     ...
     * @param data
     *            the datapoint value as an ASDU byte array (see <code>
     *            {@link ProcessEvent}.getASDU()</code>)
