...
/**
 * An adapter that works as a bridge between the logging API of calimero and
 * slf4j.
 * 
 * Calimero (the KNX I/O library used by this binding) has its own and very
 * specific logging framework, which is not compatible with any default logging
 * framework. Thus, this specific adapter is necessary in order to build a
 * bridge between the calimero logging framework and the API of slf4j.
 * 
 * All logging events from calimero are written to a single slf4j logger named
 * <code>tuwien.auto.calimero</code>.
 * 
...
public class LogAdapter extends LogWriter {
    ...
    public void write(String logService, LogLevel logLevel, String msg, Throwable t) {
        // Unfortunately, calimero does not use package names for its loggers.
        // So we cannot use "logService" and
        // we have to map all calimero-loggers to a single slf4j logger.

        // slf4j does not offer a "log(level, msg)" api, so we have to dispatch
        // in this rather ugly if-else block.
